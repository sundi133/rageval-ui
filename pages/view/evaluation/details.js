    ...
    const fetchSimulationDetails = async () => {
      try {
        ...
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_RAGEVAL_BACKEND_URL}/api/simulation/id`,
          {
            params: {
              user_id: userId,
  ...
  const triggerSimulation = async () => {
    try {
      ...
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_RAGEVAL_BACKEND_URL}/api/simulation/trigger`,
        formData
      );
      ...
      if (response.status === 200) {
        ...
        const data = response.data;
        setSuccessMessage(
          `Evaluation id ${data.simulation_id} triggered successfully.`
        );
      } else {
        // Handle error
    ...
    <main className="p-4 md:p-10 mx-auto max-w-7xl">
      <div>
        ...
        {session ? (
          <div>
            <div className="bg-white rounded-lg shadow-md p-6 mb-4">
              ...
              <div className="grid grid-cols-8 gap-2 text-sm">
                ...
                <div className="col-span-2">
                  ...
                  <div className="text-blue-500">
                    <Link
                      href={`/view/endpoints/${endpointUrlId}`}
                      rel="noopener noreferrer"
                    >
                      {endpointUrlId}
                    </Link>
            ...
            <div className="bg-white rounded-lg shadow-md p-6 mb-4">
              <Button
                ...
                disabled={isLoading}
              >
                {isLoading ? 'Submitting retrigger...' : 'Retrigger Evaluation'}
              </Button>
            </div>

            {successMessage ? (
              <div className="bg-white rounded-lg shadow-md p-6 mb-4">
                <div className="text-xl font-bold mb-4 items-center justify-center">
          ...
          <div>{!isLoading && !formSubmitted ? <div></div> : <></>}</div>
        )}
        {/* Existing code... */}
      </div>
    </main>
